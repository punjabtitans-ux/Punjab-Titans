    ...
    <div>
      ...
      <div
        className={`bg-primary  absolute right-0 ${
          toggle
            ? "translate-x-0  w-full md:w-[70%] lg:w-1/2 2xl:w-[35%] h-screen rouneded-[50%]"
            : "w-0! h-0! rouneded-[50%]"
        }
      ...
      >
          ...
          <ul>
            {headerpage.map((data, index) => (
              <li className="pb-4" key={index}>
                <Link href={data.page_link} className="text-lg md:text-2xl text-white">
                  {data.page_name}
                </Link>
              </li>
            ))}
        ...
        <div className="hidden  lg:block absolute top-[15px] right-6 cursor-pointer"
         onClick={()=>setToggle(!toggle)}
        >
          <Image
            src={crossicon}
